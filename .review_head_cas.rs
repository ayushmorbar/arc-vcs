...
pub enum CasBytes {
    ...
    Owned(Vec<u8>),
    /// Memory-mapped file for large blobs (ΓëÑ 4 096 bytes).
    Mapped(Mmap),
}
...
/// automatic deduplication ΓÇö identical content always maps to the same path.
...
impl ObjectStore {
    ...
    pub fn read_blob(&self, hash: &Blake3Hash) -> Result<CasBytes, CasError> {
        ...
        if len < 4096 {
            ...
            // SAFETY: CAS blobs are immutable once written ΓÇö no writer holds
