...
fn sign_with_loaded_identity_produces_signature_bytes() {
    let (_dir, manager) = setup();
    manager.generate("bob", "passphrase-1").expect("identity generation should succeed");
    manager.load("bob", "passphrase-1").expect("load should succeed");

    let signature =
        manager.sign(b"semantic-change-digest").expect("sign should succeed for loaded identity");
...
fn load_with_wrong_passphrase_returns_invalid_passphrase_error() {
    ...
    manager.generate("carol", "good-passphrase").expect("identity generation should succeed");
...
fn corrupted_ciphertext_returns_corrupted_ciphertext_error() {
    let (_dir, manager) = setup();
    manager.generate("dana", "good-passphrase").expect("identity generation should succeed");
...
fn generate_alias_twice_without_overwrite_errors() {
    let (_dir, manager) = setup();
    manager.generate("eve", "passphrase-2").expect("initial generation should succeed");
    let err = manager
...
fn persisted_json_does_not_leak_plaintext_secrets_or_passphrase() {
    ...
    let (_dir, manager) = setup();
    manager.generate("frank", passphrase).expect("generation should succeed");
...
fn ai_sponsorship_signature_verifies_and_tampering_is_rejected() {
    let (_dir, manager) = setup();
    manager.generate("grace", "passphrase-3").expect("sponsor identity generation should succeed");
    ...
    manager.verify_ai_sponsorship(&ai_key, &sponsorship).expect("valid sponsorship must verify");
...
fn facade_lists_and_selects_active_alias() {
    let (_dir, manager) = setup();
    manager.generate("work", "p1").expect("work identity should be generated");
    manager.generate("personal", "p2").expect("personal identity should be generated");
    ...
    let active = facade.active_alias().expect("active alias read should succeed");
