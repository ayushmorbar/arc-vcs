...
- **Transition state:** mdBook remains the legacy source renderer while
  `docs/next/` is the migration target for Next.js/MDX.
- Treat `docs/src/` as legacy content under migration and `docs/next/content/docs/`
  as the emerging Diataxis route structure.
...
When creating or migrating pages in `docs/next/content/docs/`, always include
frontmatter keys: `title`, `description`, `category`, `audience`.
