...
**Atom**
The smallest unit of change in arc. An `Atom` is one of: `Insert { at, content }`, `Delete { at }`, `SemanticsPreserving { at, from, to }`, `Blob { path, hash }`, or `Mount { path, patterns }`. Multiple atoms are grouped into a `Change`.
